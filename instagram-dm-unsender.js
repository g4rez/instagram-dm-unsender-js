...
// @license				MIT
// @copyright				Copyright (c) 2026, g4rez (https://github.com/g4rez)
// @namespace				https://github.com/g4rez/instagram-dm-unsender-js
// @author				g4rez (https://github.com/g4rez)
// @homepageURL				https://github.com/g4rez/instagram-dm-unsender-js
// @supportURL				https://github.com/g4rez/instagram-dm-unsender-js/issues
// @contributionURL				https://github.com/g4rez/instagram-dm-unsender-js
// @icon				https://www.instagram.com/favicon.ico
// @version				0.1
// @updateURL				https://raw.githubusercontent.com/g4rez/instagram-dm-unsender/userscript/instagram-dm-unsender.js
// @downloadURL				https://raw.githubusercontent.com/g4rez/instagram-dm-unsender/userscript/instagram-dm-unsender.js
...
(function (exports) {
	...
	/**
	 * When true, uses minimal delays (faster unsend / scroll / page load). Instagram may rate-limit more often.
	 * Set to false to restore the original ~1–2s spacing between unsends.
	 */
	const VERY_FAST_MODE = true;
	...
	class UIMessage extends UIComponent {
		...
		async showActionsMenuButton(abortController) {
			...
			for (let attempt = 0; attempt < 3; attempt++) {
				...
				await new Promise(resolve => setTimeout(resolve, VERY_FAST_MODE ? 20 : 100));
				...
				await new Promise(resolve => setTimeout(resolve, VERY_FAST_MODE ? 10 : 50));
			}
		...
		async unsend(abortController) {
			...
			try {
				...
				try {
					...
					doc.body.dispatchEvent(new KeyboardEvent("keydown", { key: "Escape", bubbles: true }));
					await new Promise(resolve => setTimeout(resolve, VERY_FAST_MODE ? 50 : 200));
	...
	async function loadMoreMessages(root, abortController) {
		...
		if (isAtTop()) {
			...
			await new Promise(resolve => setTimeout(resolve, VERY_FAST_MODE ? 100 : 500));
			...
			if (loader) {
				...
				await Promise.race([
					...
					new Promise(resolve => setTimeout(resolve, VERY_FAST_MODE ? 2000 : 5000))
				]);
				abortController.signal.removeEventListener("abort", abortHandler);
		...
		async getNextUIPIMessage(abortController) {
			...
			for (let pass = 0; pass < 3; pass++) {
				...
				if (isReversed) {
					...
					const step = VERY_FAST_MODE
						? (totalRange < 500 ? 50 : 220)
						: (totalRange < 500 ? 30 : 150);
					...
					for (let i = startPos; i >= minScroll; i = i - step) {
						...
						await new Promise(resolve => setTimeout(resolve, VERY_FAST_MODE ? 0 : 5));
						try {
							const messageElement = getFirstVisibleMessage(uiMessagesWrapperRoot, abortController, this.root);
							if (messageElement) {
								const uiMessage = new UIMessage(messageElement);
					...
					const step = VERY_FAST_MODE
						? (maxScroll < 500 ? 50 : 220)
						: (maxScroll < 500 ? 30 : 150);
	...
	class DefaultStrategy extends UnsendStrategy {
			...
			try {
				...
				if (this._unsentCount === 0 && !this._abortController.signal.aborted) {
					for (let retry = 1; retry <= 3; retry++) {
						...
						await new Promise(resolve => setTimeout(resolve, VERY_FAST_MODE ? 400 : 2000));
		 ...
		 * Uses adaptive delays (1–2s by default, minimal if VERY_FAST_MODE) with exponential backoff on rate limit detection.
		 */
		async #unsendNextMessage() {
			...
			try {
				...
				if (uipiMessage) {
					...
					// Adaptive delay between unsends (none in VERY_FAST_MODE; else 1–2s + jitter)
					if (this._lastUnsendDate !== null) {
						const elapsed = Date.now() - this._lastUnsendDate.getTime();
						const minDelay = VERY_FAST_MODE
							? 0
					...
					if (unsent) {
						...
						await new Promise(resolve => setTimeout(resolve, VERY_FAST_MODE ? 120 : 800));
	...
	[...this.menuElement.querySelectorAll("button")].filter(button => button !== this.unsendThreadMessagesButton).forEach(button => {
			...
			try {
				...
				this.statusElement.innerHTML = `<span style="color: red">An error occured, please open an issue</span>`;
			} finally {
